...
class SRFlowNet(nn.Module):
    def __init__(self, in_nc, out_nc, nf, nb, gc=32, scale=4, K=None, opt=None, step=None):
        ...
        self.flowUpsamplerNet = \
            FlowUpsamplerNet((128, 128, 3), hidden_channels, K,
    ...
    def forward(self, gt=None, lr=None, z=None, eps_std=None, reverse=False, epses=None, reverse_with_grad=False, clean = True,
                lr_enc=None,
                add_gt_noise=False, step=None, y_label=None, flag=False):
        if flag == True:
    ...
    # Changyeop
    def reverse_embedding(self, lr, z, y_onehot, eps_std, epses=None, lr_enc=None, clean=True, add_gt_noise=True):
        logdet = torch.zeros_like(lr[:, 0, 0, 0])
        pixels = thops.pixels(lr) * self.opt['scale'] ** 2

...
def main():
    ...
    os.makedirs(f"SRFlow/checkpoint/celeba_32_128_load", exist_ok=True)
    os.makedirs(f"SRFlow/sample/celeba_32_128_load", exist_ok=True)
    ...
    def sample_data(path, batch_size, image_size):
        ...
        loader = torch.utils.data.DataLoader(dataset, shuffle=False, batch_size=batch_size, num_workers=4)
        loader = iter(loader)
    ...
    dataset_lr = iter(sample_data('/mnt/HDD3_coursework/srdualglow/celeba_train', 4, 128 // 4))
    dataset_hr = iter(sample_data('/mnt/HDD3_coursework/srdualglow/celeba_train', 4, 128))
    ...
    dataset = torchvision.datasets.ImageFolder('./celeba_train', transform=transform)
    # print(dataset)
    # dataset = datasets.CelebA(root='./dataset', split='train', transform=transform, download=True)
    train_loader = torch.utils.data.DataLoader(dataset, shuffle=False, batch_size=4, num_workers=4)
    ...
    # model.load_network('/mnt/HDD4_collab/cycleflow/saved_models/rrdb_celeba_32_128/generator_199000.pth', model.netG.module.RRDB) # RRDB

    #### resume training
    if resume_state:
        # resume_state = torch.load('/mnt/HDD3_coursework/srdualglow/SRFlow/experiments/train_abeforeshutdown/training_state/126000.state')
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        with tqdm(range(200000)) as pbar:
            for i in pbar:
                img_lr, _ = next(dataset_lr)
                # img_lr = img_lr.to(device)

                img_hr, _ = next(dataset_hr)
                ...
                #### training
                model.feed_data({'LQ' : img_lr, 'GT' : img_hr})
                ...
                ## save images seungjae
                if current_step % opt['train']['val_freq'] == 1 and rank <= 0:
                    with torch.no_grad():
                        model.feed_data({'LQ' : img_lr, 'GT' : img_hr})
                        model.test()
                        ...
                        if hasattr(model, 'heats'):
                            for heat in model.heats:
                                for i in range(model.n_sample):
                                    ...
                                    util.save_img(sr_img, f"SRFlow/sample/celeba_32_128_load/gt_sr_{str(current_step + 1).zfill(6)}_{int(heat * 100)}_{i}.png")
                                    '''
                                    torchvision.utils.save_image(sr_img,
                                        f"SRFlow/sample/celeba_32_128_load/gt_sr_{str(current_step + 1).zfill(6)}_{i}.png"
                                        )'''
                        else:
                            sr_img = util.tensor2img(visuals['SR'])  # uint8
                            util.save_img(sr_img, f"SRFlow/sample/celeba_32_128_load/gt_sr_{str(current_step + 1).zfill(6)}_{i}.png")
                        assert sr_img is not None


                        lq_img = util.tensor2img(visuals['LQ'])  # uint8
                        util.save_img(lq_img, f"SRFlow/sample/celeba_32_128_load/gt_lr_{str(current_step + 1).zfill(6)}.png")


                        gt_img = util.tensor2img(visuals['GT'])  # uint8
                        util.save_img(gt_img, f"SRFlow/sample/celeba_32_128_load/gt_hr_{str(current_step + 1).zfill(6)}.png")
                ...
                if current_step % opt['logger']['save_checkpoint_freq'] == 1:
                    if rank <= 0:

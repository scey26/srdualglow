...
import numpy as np
from models.modules.RRDBNet_arch import RRDBNet, Discriminator
...
class SRFlowNet(nn.Module):
    def __init__(self, in_nc, out_nc, nf, nb, gc=32, scale=4, K=None, opt=None, step=None):
        ...
        self.RRDB_O = RRDBNet(in_nc, out_nc, nf, nb, gc, scale, opt)
        self.Discriminator = None
        self.Discriminator_rgb = None
        hidden_channels = opt_get(opt, ['network_G', 'flow', 'hidden_channels'])
        hidden_channels = hidden_channels or 64
        self.RRDB_training = True  # Default is true
        self.RRDB_O_training = True  # Default is true

        train_RRDB_delay = opt_get(self.opt, ['network_G', 'train_RRDB_delay'])
        set_RRDB_T_to_train = True
        set_RRDB_O_to_train = False

        if set_RRDB_T_to_train:
            self.set_rrdb_t_training(False)
        if set_RRDB_O_to_train:
            self.set_rrdb_o_training(True)
    ...
    def set_rrdb_t_training(self, trainable):
    ...
    def set_rrdb_o_training(self, trainable):
        if self.RRDB_O_training != trainable:
            for p in self.RRDB_O.parameters():
                p.requires_grad = trainable
            self.RRDB_O_training = trainable
            print('here!!')
            return True
        return False


    def forward(self, gt=None, lr=None, z=None, eps_std=None, reverse=False, epses=None, reverse_with_grad=False, clean = True,
                lr_enc=None,
                add_gt_noise=False, step=None, y_label=None, flag=False, enc_detach=False):
        if flag == True:
            return self.reverse_embedding(lr, z, y_onehot=y_label, eps_std=eps_std, epses=epses, lr_enc=lr_enc, clean = clean,
                                                add_gt_noise=add_gt_noise, enc_detach=enc_detach)
        else:
            if not reverse:
                ...
                if reverse_with_grad:
                    return self.reverse_flow(lr, z, y_onehot=y_label, eps_std=eps_std, epses=epses, lr_enc=lr_enc, clean = clean,
    ...
    def rrdbPreprocessing(self, lr, clean=True):
        if clean == True:
            rrdbResults = self.RRDB(lr, get_steps=True)
        else:
            rrdbResults = self.RRDB_O(lr, get_steps=True)
    ...
    def reverse_flow(self, lr, z, y_onehot, eps_std, epses=None, lr_enc=None, clean=True, add_gt_noise=True):
        ...
        if lr_enc is None:
            lr_enc = self.rrdbPreprocessing(lr, clean=clean)
    ...
    def reverse_embedding(self, lr, z, y_onehot, eps_std, epses=None, lr_enc=None, clean=True, add_gt_noise=True, enc_detach=False):
        logdet = torch.zeros_like(lr[:, 0, 0, 0])
        pixels = thops.pixels(lr) * self.opt['scale'] ** 2
        ...
        if lr_enc is None:
            if False: # enc_detach:
                ...
                print('here!!!!!!!!!!!!!!!!!!!!!!!!!!')
                for enc in lr_enc:
                    if lr_enc[enc] is not None:
                        lr_enc[enc] = lr_enc[enc].detach()
            else:
                lr_enc = self.rrdbPreprocessing(lr, clean=clean)

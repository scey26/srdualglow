...
class SRFlowModel(BaseModel):
    ...
    def init_optimizer_and_scheduler(self, train_opt):
        ...
        # self.optimizer_G_RGB = torch.optim.Adam(
        #     [
        #         {"params": optim_params_other, "lr": train_opt['lr_G'] / 100, 'beta1': train_opt['beta1'],
        #          'beta2': train_opt['beta2'], 'weight_decay': wd_G},
        #         # {"params": optim_params_RRDB, "lr": train_opt.get('lr_RRDB', train_opt['lr_G']),
        #         #  'beta1': train_opt['beta1'],
        #         #  'beta2': train_opt['beta2'], 'weight_decay': wd_G}
    ...
    def unified_forward(self, step, Tensor):
        ...
        self.loss_G =  0.01 * (self.loss_GAN) + self.loss_pixel
        ...
        clean_hr, _, _ =self.netG(lr=self.var_L, z=z, eps_std=0, reverse=True, reverse_with_grad=True, clean = True, flag=True, enc_detach=True)
        real_hr, _, _ =self.netG(lr=self.real_L, z=z, eps_std=0, reverse=True, reverse_with_grad=True, clean = False, flag=True, enc_detach=True)

        print('d', clean_hr.max(), real_hr.max())

        pred_real_rgb = self.netG.module.Discriminator_rgb(clean_hr).detach()
        pred_fake_rgb = self.netG.module.Discriminator_rgb(real_hr)

        self.loss_GAN_rgb = criterion_GAN(pred_fake_rgb - pred_real_rgb.mean(0, keepdim=True), valid_rgb)

        with torch.no_grad():
            for i, p in enumerate(self.netG.module.RRDB_O.parameters()):
                if i == 0:
                    print("here1 ", p.flatten()[:3])
        # self.optimizer_G_RGB.zero_grad()
        self.optimizer_PG.zero_grad()
        self.loss_GAN_rgb.backward()
        self.optimizer_PG.step()
        # self.optimizer_G_RGB.step()
        with torch.no_grad():
            for i, p in enumerate(self.netG.module.RRDB_O.parameters()):
                if i == 0:
                    print("here2 ", p.flatten()[:3])
        ...
        # -------------------------
        #  Train RGB Discriminator
        # -------------------------

        clean_hr, _, _ =self.netG(lr=self.var_L, z=z, eps_std=0, reverse=True, reverse_with_grad=True, clean = True, flag=True, enc_detach=True)
        real_hr, _, _ =self.netG(lr=self.real_L, z=z, eps_std=0, reverse=True, reverse_with_grad=True, clean = False, flag=True, enc_detach=True)

        print(clean_hr.max(), real_hr.max())

        self.optimizer_PD_rgb.zero_grad()

        pred_real_rgb = self.netG.module.Discriminator_rgb(clean_hr.detach())
        pred_fake_rgb = self.netG.module.Discriminator_rgb(real_hr.detach())
        ...
        # return nll_loss, self.loss_pixel, self.loss_GAN, self.loss_D
    ...
    def pre_forward(self, Tensor):
        ...
        z = self.get_z(heat=0, seed=None, batch_size=self.var_L.shape[0], lr_shape=self.var_L.shape)
        _, _, target =self.netG(lr=self.var_L, z=z, eps_std=0, reverse=True, reverse_with_grad=True, clean = True, flag=True)
        ...
        _, _, output =self.netG(lr=self.real_L, z=z, eps_std=0, reverse=True, reverse_with_grad=True, clean = False, flag=True)
        idt_output = self.netG.module.rrdbPreprocessing(self.var_L, clean=False)[cur_layer] 
        ...
        return self.loss_GAN, self.loss_pixel, self.loss_D

...
class LinearNet(nn.Module):
    def __init__(self, in_features, out_features, condition):
        ...
        # last_layer = nn.Linear(in_features=48, out_features=self.out_features) # It should be modified!
    ...
    def __init__(self, cond_shape, inp_shape, do_lu=False, initial_bias=None):
        ...
        if self.do_lu:
            ...
            self.linear_net = LinearNet(in_features=conv_out_flat_length,
                                        ...
                                        condition='w - LU')
            # self.linear_net.linear_net[-1].bias.data.copy_(initial_bias)  # init with flattened LU and s elements
        else:
            self.linear_net = LinearNet(in_features=conv_out_flat_length,
